...
1. **朝のダイジェスト配信（v1 / cron）**: 平日 09:50 JST に WWD JAPAN / FASHIONSNAP / PR TIMES / 美的 / VOCE / 商業界（通販化粧品メーカー cat19）から記事を集め（@cosme は 2026-06-26 にサイト構造変化で一旦 disable）、メビウス文脈で関連度の高いものを最大 5 件、Slack の 1 投稿にまとめて流す。**発火は ConoHa crontab + `scripts/trigger-daily.sh` 経由で workflow_dispatch を叩く**（GitHub Actions の schedule trigger は実測 4 時間遅延のため 2026-06-26 廃止、詳細は `docs/external-cron.md`）。土日 / 日本の祝日 / 年末年始（12/29〜1/3）は `src/calendar.ts` の `shouldSkipToday()` で自動スキップ。手動で祝日でも配信したい時は `FORCE_RUN=true pnpm start:local`
