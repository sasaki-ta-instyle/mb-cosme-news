...
1. **朝のダイジェスト配信（v1 / cron）**: 平日 09:05 JST に WWD JAPAN / FASHIONSNAP / PR TIMES / 美的 / VOCE から記事を集め（@cosme は 2026-06-26 にサイト構造変化で一旦 disable）、メビウス文脈で関連度の高いものを最大 5 件、Slack の 1 投稿にまとめて流す（GitHub Actions schedule）。土日 / 日本の祝日 / 年末年始（12/29〜1/3）は `src/calendar.ts` の `shouldSkipToday()` で自動スキップ。手動で祝日でも配信したい時は `FORCE_RUN=true pnpm start:local`
